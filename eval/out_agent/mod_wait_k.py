...
class ModWaitKOutAgent(OutAgentBase):
    def __init__(self, model_path: str, hparam_path: str, k: int = 3, beam_w=1) -> None:
    ...
    def put(self, batch: Sequence[Any]) -> list[int]:
        ...
        with torch.no_grad():
            ...
            if input_ids[0] == [1] or self.buffer[-1] == 2:
                return self.buffer

            if kana == 2:
                step_count = 0
                while self.buffer[-1] != 2:
                    if step_count > 100:  # TODO
                        return self.buffer
